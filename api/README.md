...
> Plataforma completa de **Casa Inteligente e Assistente Pessoal com Inteligência Artificial**, equipada com agente conversacional por voz contínua, automações residenciais IoT (MQTT), visão computacional com reconhecimento facial, integração com serviços Google e Telegram, e pesquisa com transcrição de tutoriais do YouTube.
...
O **Casa Beta** é uma solução inteligente e modular desenvolvida sobre **FastAPI**, **LangChain** e **SQLite**. O sistema integra um painel de controle 3D em tempo real com um agente de inteligência artificial multilíngue com voz neural natural, capaz de executar ações no mundo real e gerenciar a rotina do usuário.
...
| `consultar_perfil_usuario` | Consulta memória de preferências, hábitos e dados pessoais do morador. |
...
- **Áudio & Vídeo**: `edge-tts` (Microsoft Neural Voices), `youtube-transcript-api`, OpenCV (`cv2`).
...
├── agent.py                 # Orquestração do agente LangChain e prompt
├── auth.py                  # Autenticação JWT, login, registro e segurança
├── database.py              # Camada de banco de dados SQLite (CRUD)
