...
(function () {
    ...
    function createFreshRecognition() {
        ...
        rec.onresult = (event) => {
            ...
            // BARGE-IN: Interrupção de áudio APENAS se o usuário chamar o nome cadastrado do agente (Wake Word)
            if (isSpeakingTTS || isAudioPlayingActive || speechQueue.length > 0) {
                ...
                // O áudio SÓ É interrompido se a Wake Word (nome do agente) for detectada
                if (isWake) {
                    console.log("[VoiceAgent Barge-In por Wake Word]:", currentText);
                    silenciarAudio(false);
                    latestHeardText = "";

                    const isStopCmd = isStopTalkingCommand(command);
                    if (command.length > 1 && !isStopCmd) {
                        ...
                        // Chamou apenas o nome (ex: "Sexta-feira") ou comando de parar (ex: "Sexta-feira, pare")
                        clearTimeout(wakeWordTimer);
                        isWaitingCommandAfterWakeWord = true;
                        showVoiceHud();
                        setVoiceStatus("👂", "Áudio interrompido. Ouvindo...", true);
                        setVoiceTranscript(`"${agentName}..." (Áudio parado)`, false);
                ...
                // Ignora ruídos, eco do próprio TTS e palavras sem o nome do agente
                return;
            }
    ...
    function getAgentFeedbackPhrase(commandText) {
        ...
        // 11. OSINT & Sherlock / Holehe / PhoneXtract
        if (/\b(investigar|investigacao|osint|sherlock|holehe|phonextract|telefone|telefones|celular|numero de telefone|ddd|operadora|dossie|rastrear|redes sociais)\b/.test(raw)) {
